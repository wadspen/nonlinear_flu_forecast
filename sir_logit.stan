...
functions {
  ...
  vector sir2(real t, 
      ...
      // vector[3] state = [dS_dt, dI_dt, dR_dt];
      // return state;
      // return {dS_dt, dI_dt, dR_dt};
      return dydt;
  }
...
parameters {
  ...
  real<lower=0,upper=1 - S0> I0[n_seasons];
...
model {
  ...
  mu[n_weeks - 1] ~ normal(0, sqrt(1/sigmuT2));
  ...
  for (i in 1:M) {
    //print((1 + (exp(mu[weeks[i]]) - 1)));
    // print("IS ", IS[i]);
    // print("mu ", mu[weeks[i]]);
    if (IS[i] < 0) {print(IS[i]);
                    print("we are at week ", weeks[i]);}
    
     ili[i] ~ beta_proportion(
                IS[i]*exp(muu[weeks[i]])/
                (1 + (exp(muu[weeks[i]]) - 1)*IS[i]), 
                              phi);
  }
  //print(seg)
...
generated quantities {
  ...
  ypm = ode_rk45(sir2, yp, t0, tp, beta[ps], gamma[ps], N);
