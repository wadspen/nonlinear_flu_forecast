...
flu season by the year in which the outbreak began, i.e. the 2020 season 
...
\cite[]{wadsworth2024bas}. These plots show flu outbreak trajectories 
...
used for modelling disease outbreaks which may capture ILI behavior. 
...
Here 
$p(\xi | \textbf{ILI}_w)$ 
is posterior distribution from the ILI model (\ref{eq:ili_model})
...
In the second stage, the posterior distribution
for parameters of the hospitalization 
...
(MCMC) sampling.
In the first or ILI stage, $K$ MCMC draws
$(\widetilde{ILI}_{w +i}^{(1)}, ..., \widetilde{ILI}_{w +i}^{(K)})$
from the forecast model were produced,
and in the second or hospitalization stage
$K$ draws $(\psi^{(1)}, ..., \psi^{(K)})$ 
from the hospitalization posterior 
