...
The competition lasted from the fall of 2013 through spring of 2014, and
...
Besides the 2020-21 season, FluSight has been operated annually and 
researchers outside 
the CDC have been invited to participate. From hereon, we denote a 
flu season by the year in which the outbreak began, i.e. the 2020 
season outbreak began in 2020 and lasted until spring 2021.
Initially the target data for 
...
osthus2021multiscale, osthus2019dynamic, ulloa2019}, and in their 
...
probabilistic framework. An additional forecast modeling class used in FluSight 
...
FluSight. Additionally the target of the flu forecasts 
...
\cite[]{mathis2024evaluation,healthdata2024covidts}. This in part was 
as a result of 
...
introduced challenges which required thoughtful selection of model 
...
model hospitalizations as a linear function of ILI. 
The flu hospitalization forecasts are a linear mapping of ILI forecasts
allowing for ILI data from many seasons to be exploited
and assist in forecasting hospitalizations.
To produce hospitalization forecasts, a scheme similar to 
the Bayesian cut is used where forecasts from the posterior predictive
distribution of an ILI model are plugged into the hospitalization model
to produce a posterior predictive hospitalization forecast distribution.
We show that this scheme produces accurate and reasonably well calibrated
...
they outperform all but one of the FluSight models.
...
defined.
 ...
 used proper scoring rules \cite[]{gneiting2007strictly} are used for 
 comparing the forecasts. Forecasting of the 2023 flu outbreak along with 
 assessment and comparison of 20 FluSight models is performed in 
...
In this section we introduce, define, and visually evaluate
ILI and hospitalization 
data. ILI and hospitalization data have been the object 
...
2022 season. Both of 
these data were collected at the state, territorial, and national levels and 
...
and the US national level. We will refer to forecast targets throughout this 
...
The US Outpatient Influenza-like Illness Surveillance Network collects 
...
Prior to the 2021 season, the definition included 
...
and census levels are available from the 1997 flu season up to the current 
...
The only seasons when there was not a peak at week 22 were 2022, when 
the season peak occurred particularly early, and 2020 which was greatly 
...
will be referred to as hospitalizations.
...
trend similar to the ILI plots in that during the early 
...
\cite[]{wadsworth2025bas}. These plots show flu outbreak trajectories 
...
The typical behavior of the ILI data to begin low, rise to a peak, and then 
fall motivates the use of a nonlinear function which follows 
a similar trajectory. Compartmental models are standard mathematical models 
used for modeling disease outbreaks which may capture ILI behavior. 
One important compartmental model is the 
...
generalization, the model may incorporate any appropriate linear or
nonlinear function, 
...
models using a scheme similar to the 
...
selection for modeling. Under the parameterization of the Beta distribution 
...
over seasons. Herein we also adopt the hierarchical modeling so as to borrow 
...
it is 
reasonable to use the SIR model to partially capture ILI, although it is
unlikely to completely capture the true data generating mechanism. Thus the 
use of modeling discrepancy.
...
illustrate the systematic discrepancy from a fitted function to ILI  data.
Figure 
...
fits for each season were made by computing the maximum likelihood estimate 
...
line is the average discrepancy over all seasons. The lines show that the ASG 
function typically underpredicts week 22 and overpredicts week 23. Perhaps 
for other weeks, week 30 for example, there tends to be additional systematic 
...
(\ref{eq:asg_function_rep}) difficult to 
...
is specific to season $s$. To assist in estimating $\gamma_w$,
...
In model (\ref{eq:final_hosp}), the data is limited to the ILI and 
...
in the bottom row. We found that models with more than 1 autoregressive
lag showed little or no forecast
...
\begin{figure}[hbt!]
    ...
    $\phi = 0.6$, and for log hospitalizations $\phi = 0.77$. Lines through
    the points follow the weekly progression of influenza.}
...
In the simulation study performed in section \ref{sec:simulation2} 
$D_s$ belongs to 
the normal distribution family, and for for analyzing the 2023 hospitalization
data presented in section \ref{sec:analysis} models were considered where 
$D_s$ was LST or where hospitalization counts were log transformed 
and the transformed
data were modeled as normally distributed.
...
model (\ref{eq:final_hosp}) making it more difficult to produce forecasts
...
selected for the ILI data model under both the SIR and ASG models 
...
estimated is 
$\psi_s = (\alpha_{0s}, \alpha_{1s}, \alpha_{2s}, \phi, \sigma_{\epsilon_s}, \omega_s)$. 
...
hierarchical prior $\theta$ to be the mean of MLEs for $\lambda_s$ across
...
the ILI model (\ref{eq:ili_model}) and hospitalization model
(\ref{eq:final_hosp}) were fit separately via Bayesian posterior updating. 
...
Here 
$p(\xi | \textbf{ILI}_w)$ 
is the posterior distribution of the ILI model (\ref{eq:ili_model})
...
model (\ref{eq:final_hosp}) given observed
...
(\ref{eq:final_hosp}) to produce the approximate hospitalization forecast
distribution in the bottom line of (\ref{eq:hosp_post_pred}). This has 
similarities to the procedure used in section 3.2.2 of 
\cite[]{sahu2014hierarchical}, which uses it to propagate the uncertainty of 
temperature forecasts into forecast model for hospitalizations.
Here $k \in \{1, ..., K\}$, 
and forecasts were obtained for weeks $i \in \{1, 2, 3, 4\}$.
In the analyses in sections \ref{sec:simulation2} and \ref{sec:analysis}
...
(\ref{eq:final_hosp}) was not directly influenced by ILI model 
...
Both component models were fit via MCMC sampling using the 
...
implements Hamiltonian Monte Carlo sampling with the No-U-turn 
...
\cite[]{wadsworth2025bas}.

Occasionally the draws making up the approximate forecast
distribution
% $\{H^{(k)}_{s,w + i}\}^K$ 
included a small number of negative 
values,
which do not make sense when the forecast is meant to predict total
...
section \ref{sec:analysis}, all negative values from 
the hospitalization model (\ref{eq:hosp_post_pred})
were 
...
% steps were repeated $K$ times where $K$ is an integer for the number of desired 
...
a model assumption we make but only a simplifying assumption for simulating
the data.
...
forecasts --the forecast format used in both FluSight and the 
...
The WIS is 
defined in (\ref{eq:wis2}) where $Q$ is a forecast represented by all 
...
model is the closest to being well calibrated, especially at the highest 
...
% function. 
% Additional
% plots in the supplemtary material \cite[]{wadsworth2025bas} further show that 
...
and forecasts were made at the state and national levels. The first week of 
...
% Nearly teams participated and forecasts from over 40 models were submitted throughout the duration of the season. 
 ...
 log-weighted interval score (LWIS). The LWIS is defined the same as the WIS
...
as most frequently
making the sharpest predictions, particularly later in the season. 
...
For the
remainder of this section, the focus is on forecasts made by coupling the
...
models including ASG and SIR ILI models coupled with other variations of the
hospitalization model where the ASGD\_NORM2 showed the best results
and the SIRD\_NORM2 performed nearly as well as the ASGD\_NORM2.
...
differences between 
previous hospitalizations. The baseline forecast model was
similar to the baseline forecast model used in
previous flu forecasting seasons and in the COVID-19 hub 
\cite[]{mathis2024evaluation, Cramer2022-hub-dataset}.

Figure \ref{fig:lwis_cover_sum} shows forecast results for the 2023 flu 
hospitalization forecasts for all 15 models being compared but with results
...
and the baseline model highlighted. 
...
\begin{figure}[hbt!]
    ...
    \includegraphics[scale = .6]{Images/lwis_cover_sum2.png}
    \caption{a) Log-weighted interval score (LWIS) averaged 
    over location and horizon for each week during the season for the 21 models being compared. 
...
We call this the relative LWIS (RLWIS). A value smaller than
1 indicates the ASGD\_NORM2 model outperforms the competing model. A value 
higher than 1 indicates the ASGD\_NORM2 is outperformed. A grey tile indicates
...
the plot shows the ASGD\_NORM2 performed poorly in Montana relative to most other
models.

Table \ref{tab:fin_analysis_stats} gives the numerical values for RLWIS 
...
with another model.
Along with the plots in this section, it 
...
% \begin{adjustwidth}{-1.2cm}{-1.5cm}
    ...
    \centering
    \makebox[\linewidth][l]{
    \includegraphics[scale = .5]{Images/state_and_date_lwis2.png}
    }
    % \end{adjustwidth}
...
see figure \ref{fig:lwis_cover_sum} c)), and percentage of forecasts 
submitted for all 
...
% models with over 80\% submissions and 
% the ASGD\_NORM2 and SIRD\_NORM2 models. 
% Summaries include mean weighted interval
...
% \begin{tabular}{lrrrrrrrr}
%   % \hline
%                               & MWIS & MLWIS & MSEC  & \% Forcs & RWIS & RLWIS\\ 
%   \hline 
% ASGD\_NORM2                   & 62   & 0.34  & 0.001 & 100      & 1    & 1 \\ 
%   PSI-PROF                    & 63   & 0.35  & 0.003 & 100      & 0.90 & 0.97 \\ 
%   SIRD\_NORM2                 & 67   & 0.36  & 0.001 & 100      & 0.93 & 0.95 \\
%   CEPH-Rtrend\_fluH           & 69   & 0.39  & 0.004 & 100      & 0.81 & 0.86 \\ 
%   FluSight-baseline           & 85   & 0.41  & 0.023 & 100      & 0.63 & 0.82 \\ 
%   UM-DeepOutbreak             & 83   & 0.46  & 0.010 & 99       & 0.64 & 0.72 \\ 
%   cfarenewal-cfaepimlight     & 76   & 0.37  & 0.001 & 99       & 0.72 & 0.92 \\ 
%   MOBS-GLEAM\_FLUH            & 66   & 0.35  & 0.021 & 98       & 0.87 & 0.95 \\ 
%   LosAlamos\_NAU-CModel\_Flu  & 242  & 0.64  & 0.153 & 93       & 0.21 & 0.53 \\ 
%   UGuelph-CompositeCurve      & 132  & 0.52  & 0.018 & 93       & 0.43 & 0.62 \\ 
%   LUcompUncertLab-chimera     & 84   & 0.43  & 0.004 & 92       & 0.67 & 0.71 \\ 
%   UGA\_flucast-INFLAenza      & 74   & 0.32  & 0.003 & 90       & 0.77 & 1.07 \\ 
%   cfa-flumech                 & 101  & 0.41  & 0.019 & 89       & 0.58 & 0.85 \\ 
%   UGA\_flucast-Copycat        & 76   & 0.35  & 0.012 & 87       & 0.81 & 0.98 \\ 
%   GT-FluFNP                   & 105  & 0.48  & 0.046 & 86       & 0.45 & 0.73 \\ 
%   % UNC\_IDD-InfluPaint         & 107 & 0.35 & 16 & 0.24 & 0.043 & 78 & 0.58 & 0.95 \\ 
%   % NU\_UCSD-GLEAM\_AI\_FLUH    & 98 & 0.39 & 22 & 0.24 & 0.014 & 74 & 0.68 & 0.83 \\ 
%   % SigSci-CREG                 & 59 & 0.49 & 12 & 0.31 & 0.018 & 71 & 0.62 & 0.74 \\ 
%   % PSI-PROF\_beta              & 77 & 0.32 & 18 & 0.20 & 0.004 & 67 & 0.87 & 0.93 \\ 
...
\label{sec:conclusion}
 ...
 specifically incorporate the smooth ASG function.
 ...
 linear or quadratic function of ILI. Utilizing the ILI data which has been
 ...
 exploit the ILI data. Including model discrepancy was an important piece of 
...
well across the whole season, there were a small number of weeks or locations
where performance was not as good.
It should not be assumed that ASGD\_NORM2 forecasts would perform
...
disease outbreak in one state will share characteristics with the outbreak in 
neighboring states, and indeed the behavior of ILI and hospitalizations are more
...
Further extensions of the models herein could include explicit modeling of 
spatial relationships between states and inclusion in an ensemble of other
forecast models. Additionally, it has been shown that meteorological variables
