...
print(info_list$theta_s)
  ...
  samps <- mod2$sample(data = stan_dat,
                      chains = 1,
                      # seed = 0,
                      adapt_delta = .999,
                      init = list(init),
  ...
  num <- ",13]"
  lnum <- nchar(num)
  postpr <- draws[,which(str_detect(colnames(draws),pattern = 'pred_ili'))]
  ...
  pi <- as.data.frame(predictive_interval(as.matrix(postpr), prob=.00000095))
  ...
  ILINet %>%
    ...
    ggplot() +
    # geom_point(aes(x=(season_week), y = log(value))) +
