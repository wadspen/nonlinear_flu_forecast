...
get_ili_data <- function(lag = 2) {
  ...
  pops <- pops %>% 
    mutate(count_rate2 = count_rate1*2) %>% 
...
make_stan_data <- function(dat1, dat2, ili_seasons,
  ...
  if (traj == 'asg') {
    ...
    theta_s <- all_season_region_mles %>% 
      select(beta, eta, mu, sigma1, sigma2, kappa) %>% 
      as.matrix()
    theta_s <- rbind(theta_s, pars[-1])
    theta_s[length(seasons),] <- apply(as.matrix(theta_s[1:(length(seasons) - 1),]), 
                                       MARGIN = 2, FUN = mean)
  }
