...
lambda[1] ~ normal(alpha0[hosp_seasons[1]] +
...
hosp[1] ~ poisson(exp(lambda[1]));

for (i in 2:HM) {
        lambda[i] ~ normal(alpha0[hosp_seasons[i]] + 
                                 alpha1[hosp_seasons[i]]*(count_rate*ili_ps[i]) +
                                 //alpha2[hosp_seasons[i]]*(count_rate*ili_ps[i])^2 +
				 alpha3[hosp_seasons[i]]*lambda[i-1], 
                                             count_rate*sigma_epsilon[hosp_seasons[i]]);  
         
         hosp[i] ~ poisson(exp(lambda[i]));
