...
transformed parameters{
  ...
  mu1[1] = -sum(mu);
  muu = to_vector(append_array(mu1, mu));
  
  upsilon1[1] = 0; //-exp(beta[n_seasons])/(1 + exp(beta[n_seasons]));
  ups = to_vector(append_array(upsilon, upsilon1));
...
model {
  ...
  //     (IS[i]+.000001)*exp(muu[weeks[i]] + ups[weeks[i]])/
  //       (1 + (exp(muu[weeks[i]] + ups[weeks[i]]) - 1)*(IS[i]+.000001)), 
  //     kappas[all_seasons[i]]);
  //     
  // }
  
  ili[1:(M - cur_yr_n_weeks)] ~ beta_proportion(
      ((ISv[1:(M - cur_yr_n_weeks)]+.000001).*exp(muu[weeks[1:(M - cur_yr_n_weeks)]]))./
        (1 + (exp(muu[weeks[1:(M - cur_yr_n_weeks)]]) - 1).*(ISv[1:(M - cur_yr_n_weeks)]+.000001)), 
      kappas[all_seasons[1:(M - cur_yr_n_weeks)]]);
  
  
  // for (i in (M - cur_yr_n_weeks + 1):M) {
  //       
  //   ili[i] ~ beta_proportion(
  //     (IS[i]+.000001)*exp(muu[weeks[i]])/
  //       (1 + (exp(muu[weeks[i]]) - 1)*(IS[i]+.000001)), 
  //     kappas[all_seasons[i]]);
  //     
  // }
  
  ili[(M - cur_yr_n_weeks + 1):M] ~ beta_proportion(
      (ISv[(M - cur_yr_n_weeks + 1):M]+.000001).*exp(muu[weeks[(M - cur_yr_n_weeks + 1):M]] + 
        ups[weeks[(M - cur_yr_n_weeks + 1):M]])./
        (1 + (exp(muu[weeks[(M - cur_yr_n_weeks + 1):M]] + 
        ups[weeks[(M - cur_yr_n_weeks + 1):M]]) - 1).*(ISv[(M - cur_yr_n_weeks + 1):M]+.000001)), 
      kappas[all_seasons[(M - cur_yr_n_weeks + 1):M]]);
...
generated quantities {
  ...
  array[cur_yr_n_weeks + 5] real pred_ili;
  ...
  // for(j in 1:n_seasons) {
    for (i in 1:(cur_yr_n_weeks + 5)) {
      ...
      pred_ili[i] = beta_proportion_rng(
        (IP[i]+.000001)*exp(muu[i] + ups[i])/
          (1 + (exp(muu[i] + ups[i]) - 1)*(IP[i]+.000001)), 
        kappas[n_seasons]);
        
      pred_ili_asg[i] = beta_proportion_rng((IP[i]+.000001), 
                                            kappas[n_seasons]);
