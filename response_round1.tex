...
revised manuscript. Our responses to each reviewers particular comments are
\emph{italicized}, and specific changes/additions to the main manuscript are 
indented.
\\~\\
...
\begin{enumerate}[-]
  ...
  \item Section 3.7. End of of first paragraph: ``..., but propagation of 
  uncertainty   in ILI forecasts was stil permitted in the estimating of 
  ...
  \\~\\
  \emph{To clarify how uncertainty was propogated, we consildated sections
  ...
  \begin{quote}
        ...
        \emph{\indent In the second stage, the posterior distribution
        ...
        \[
         \widetilde{H}_{s,w + i}^{(k)} = \alpha_{0s}^{(k)} + 
                                          ...
                                          \epsilon_{s,w + i}^{(k)}
        \]
        \emph{where $k \in \{1, ..., K\}$. The hospitalization draws were then combined into
  ...
  \emph{The following statements were added to page 17.}
  \begin{quote}
  \emph{
...
\normalsize
\\~\\
\noindent The authors have addressed all my comments from the first round. I only have two remaining points
on the extended empirical analysis on FluSight.

\begin{enumerate}[1.]

  \item The authors mention the difficulty of missing submissions during the FluSight season (hence the
    ...
    \\~\\
    \emph{The columns RLWIS and RWIS in table 2 correspond directly to the 
  ...
  \item It is uncommon to report median scores as their statistical properties are unclear. As mean score
    are also there I'm not fundamentally opposed to reporting them, but in my opinion mean scores
    alone are sufficient.  
    \\~\\
    \emph{Thank you for this comment. Because of it, we removed the median scores
...
I appreciate the hierarchical modeling 
...
\\~\\
\emph{We appreciated this comment and the included reference. We used the reference
...
In conclusion, I do think that the authors have done a good job of analysis and 
...
\\~\\
\emph{In the simulation study in section 4 of the first version of this paper, 
scoring was
