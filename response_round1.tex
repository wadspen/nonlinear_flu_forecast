...
\begin{enumerate}[-]
  ...
  \begin{quote}
        ...
        forecasts were obtained from the ILI forecast model in (7).
        Here 
        $p(\xi | \textbf{ILI}_w)$ 
        is posterior distribution from the ILI model (1)
        ...
        The ILI forecast model in (7) was then combined with 
        the hospitalization posterior distribution
        to produce a hospitalization forecast $\widetilde{H}_{w + i}$ 
        for week $w + i$.
        Both the ILI forecast model in (7) and the hospitalization
        ...
        distribution in the bottom line of(8). 
        Here $k \in \{1, ..., K\}$, 
