...
The term $\gamma_w$, where $w$ is the season week, is included in 
...
\cite[]{raftery2010online,ljung1987system}. Figure \ref{fig:lag_scatter}
...
The models were fit via Markov chain Monte Carlo (MCMC) sampling using the 
...
Model comparison was done by calculating the continuous ranked probability 
score (CRPS) for each forecast. The CRPS is a proper scoring rule which 
evaluates the forecast distribution function. Proper scoring rules are the 
...
\cite[]{gneiting2007strictly, gneiting2014probabilistic}. The CRPS is defined 
...
boxplots of the CRPS for the 500 simulation replicates faceted by week and 
horizon. 
...
\begin{figure}[hbt!]
    ...
    \caption{Boxplots of continuous ranked probability score (CRPS) for the 
    four ILI models over all seasons in the simulation study faceted by horizon 
    (x-axis) and week (y-axis). Horizons include 1-4 week ahead forecasts and 
    weeks include weeks 14, 20, 26, 32, and 38 of the flu season}
...
In this section we apply the forecast models to make forecasts of the 2023 
...
% Nearly teams participated and forecasts from over 40 models were submitted throughout the duration of the season. 
 Primary scores for evaluating each forecast were the weighted interval score 
 (WIS), the log-weighted interval score (LWIS), and the relative weighted 
...
The WIS is a proper scoring rule used for scoring quantile or interval 
forecasts 
...
single interval as defined in (\ref{eq:is}). The goal of the forecaster is to 
minimize the WIS. The LWIS is the same as the WIS except that it is evaluated 
